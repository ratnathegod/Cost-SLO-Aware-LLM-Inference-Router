...
func Load() Config {
	...
	if v, err := strconv.Atoi(getenv("MOCK_MEAN_LATENCY_MS", "")); err == nil && v > 0 { cfg.MockMeanLatencyMs = v }
	if v, err := strconv.Atoi(getenv("MOCK_P95_LATENCY_MS", "")); err == nil && v > 0 { cfg.MockP95LatencyMs = v }
	if v, err := strconv.ParseFloat(getenv("MOCK_ERROR_RATE", ""), 64); err == nil && v >= 0 && v <= 1 { cfg.MockErrorRate = v }
	if v, err := strconv.ParseFloat(getenv("MOCK_COST_PER_1K_TOKENS_USD", ""), 64); err == nil && v >= 0 { cfg.MockCostPer1kUSD = v }
	return cfg
}
