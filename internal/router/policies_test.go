...
func TestCheapest(t *testing.T) {
    ...
    if got == nil || got.Name() != "a" { t.Fatalf("want a, got %v", got) }
}
...
func TestFastestP95(t *testing.T) {
    ...
    // record latencies
    for i := 0; i < 50; i++ { a.Stats().Record(50, false); b.Stats().Record(100, false) }
...
func TestSLOBurnAware(t *testing.T) {
    ...
    for i := 0; i < 20; i++ { a.Stats().Record(100, true) } // high error on cheapest
    e := NewEngine([]*providers.ResilientProvider{a, b})
    got := e.Choose("slo_burn_aware", "")
    if got == nil || got.Name() != "b" { t.Fatalf("want b, got %v", got) }
}
...
func TestCanaryRolloutAndRollback(t *testing.T) {
    ...
    primaryCount := 0
    for i := 0; i < 200; i++ {
        if e.Choose("canary", "").Name() == a.Name() { primaryCount++ }
    }
    if primaryCount < 100 { t.Fatalf("expected rollback keeping majority on primary, primaryCount=%d", primaryCount) }
}
