...
func (e *Engine) cheapest(model string) *providers.ResilientProvider {
    ...
    for _, p := range ps[1:] {
        if c := p.CostPer1kTokensUSD(model); c < bestCost { best = p; bestCost = c }
    }
    return best
}
...
func (e *Engine) Choose(policy string, model string) *providers.ResilientProvider {
    ...
    case Canary:
        ...
        if primary == nil || candidate == nil { return primary }
        ...
        if providerName == e.canary.candidate {
            ...
            if calls%200 == 0 {
                ...
                for _, p := range e.provs {
                    if p.Name() == providerName { cand = p; break }
                }
                if cand != nil {
